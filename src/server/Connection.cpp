...
bool Connection::writeResponse() {
    ...
    while (totalSent < _response.size()) {
        ssize_t sent = write(_fd, _response.c_str() + totalSent, _response.size() - totalSent);
        if (sent == -1) {
    ...
    // If not all data sent, you need to call writeResponse again when POLLOUT is set
    return (totalSent == _response.size());
}
