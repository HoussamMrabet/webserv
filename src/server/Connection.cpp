...
void requestInfo(const std::string& ip, const std::string& port, int status, const std::string& method, const std::string& path, const std::string& version) {
    time_t now = time(0);
...
bool Connection::writeResponse(){ // check if cgi or not, if cgi call cgiResponse!!!
    ...
    requestInfo(_server.getListen().begin()->first, \
                _server.getListen().begin()->second, \
                _request->getStatusCode(), \
                _request->getStrMethod(), \
                _request->getUri(), \
                _request->getHeader("httpVersion")); // fix ip port and http
    // requestInfo("127.0.0.1", 8080, _request->getStatusCode(), _request->getStrMethod(), _request->getUri(), "HTTP/1.1"); // fix ip port and http
